...
- [~] T023 [P] [US2] File-level rule predicates in `Sources/FinanceWorkspaceKit/Validation/Rules/FileRules.swift` — **partial**: missing-required-file (VAL-FILE-001) wired; invalid header/date/decimal/enum/missing-front-matter arrive via lifted parse warnings (done). **Pending**: unknown file type, invalid file name, duplicate monthly file.
- [~] T024 [P] [US2] Cross-file rule predicates in `Sources/FinanceWorkspaceKit/Validation/Rules/CrossFileRules.swift` — **partial**: generic reference checks (VAL-CROSS-001..008) + duplicate transaction ID (010) wired; delete-with-reference is lookups-only by design (clarify N1). **Pending**: missing benchmark data (009), orphan note link (011).
- [~] T025 [P] [US2] Domain rule predicates in `Sources/FinanceWorkspaceKit/Validation/Rules/DomainRules.swift` — **partial**: balanced-group net-zero (VAL-DOMAIN-005) + gross/net reconciliation (006) wired. **Pending**: budget-period-without-rows (001), goal-contribution-without-goal (002), asset-without-account (003), trade-without-asset (004), tax-payment-outside-year (007), business-txn-unknown-group (008).
