...
- [ ] T019 [P] [US2] Test: valid fixture ⇒ zero errors and zero false-positive warnings (SC-003) in `Tests/FinanceWorkspaceKitTests/ValidationTests.swift`
- [ ] T020 [P] [US2] Test: defect-seeded fixture ⇒ each rule fires exactly once with correct `VAL-<TIER>-<NNN>` id, tier, severity, repair class (SC-003) in `Tests/FinanceWorkspaceKitTests/ValidationCatalogTests.swift`
- [ ] T021 [P] [US2] Test: parse/normalization warnings surface in `ValidationResult` as file-level issues (unified stream, FR-009a) in `Tests/FinanceWorkspaceKitTests/UnifiedIssueStreamTests.swift`
- [ ] T022 [US2] Implement `RuleCatalog` — one rule per condition with `VAL-<TIER>-<NNN>` ids + the locked classification defaults, per `contracts/validation-rule-catalog.md`, in `Sources/FinanceWorkspaceKit/Validation/RuleCatalog.swift` (resolves the open "validation rule catalog" item)
- [ ] T023 [P] [US2] Implement file-level rule predicates (missing required file, unknown file type, invalid name, duplicate monthly file, invalid header/date/decimal/enum, missing required front matter) in `Sources/FinanceWorkspaceKit/Validation/Rules/FileRules.swift`
- [ ] T024 [P] [US2] Implement cross-file rule predicates (unknown account/account-group/category/asset/liability/portfolio/sleeve/goal references, duplicate transaction ID, orphan note link) plus the inbound-reference *lookups* for the Phase-6 delete-reassign policy — no static rule fires for delete-with-reference in this phase — in `Sources/FinanceWorkspaceKit/Validation/Rules/CrossFileRules.swift`
- [ ] T025 [P] [US2] Implement domain rule predicates (budget period w/o rows, goal contribution w/o goal, asset w/o account, trade w/o sending/receiving asset, balanced group ≠ 0, gross/net group reconciliation, tax payment outside year, business txn unknown group) in `Sources/FinanceWorkspaceKit/Validation/Rules/DomainRules.swift`
- [ ] T026 [US2] Implement `ValidationEngine.validate(_:)` — full pass (file→cross-file→domain), group by severity, **lift parse warnings** into the issue stream, no false positives on sparse data (FR-009, FR-009a, FR-009b, FR-010, FR-011) in `Sources/FinanceWorkspaceKit/Validation/ValidationEngine.swift`
