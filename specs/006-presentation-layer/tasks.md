...
- [X] T001 Add `FinanceWorkspaceAppTests` test target to `Package.swift` (depends on
...
- [X] T002 [P] Create `Sources/FinanceWorkspaceApp/DesignSystem/Tokens.swift` — every DESIGN.md
...
- [X] T003 [P] Create `Sources/FinanceWorkspaceApp/DesignSystem/Typography.swift` — the 8-step
      type scale (page-title…caption) with `.monospacedDigit()` on all numeric styles
- [X] T004 [P] Create `Sources/FinanceWorkspaceApp/DesignSystem/Components/` base styles —
      `StatusChipStyle`, `TagStyle`, primary/secondary/ghost `ButtonStyle`s, `PanelView` chrome
      (panel-head + panel-body), filter-pill style
- [X] T005 Run the `design-adherence` gate + `design-token-sync` audit over the new token layer
...
- [X] T006 [P] Create `Sources/FinanceWorkspaceApp/UI/Shared/PresentationModels.swift` —
      `SourceRef`, `Provenance`, `Delta`, typed-state display helpers, and tabular
      currency/percent/date formatters (per data-model.md)
- [X] T007 Create `WorkspaceProjections` immutable snapshot (dashboard, per-module projections,
      issues, retained `WorkspaceContext`, builtAt/asOf) in
      `Sources/FinanceWorkspaceApp/ProjectionStore.swift`
- [X] T008 Implement `ProjectionStore` in `Sources/FinanceWorkspaceApp/ProjectionStore.swift` —
...
- [X] T009 [P] Write `Tests/FinanceWorkspaceAppTests/ProjectionStoreTests.swift` — atomic-swap
...
- [X] T010 [US1] Create `Route` enum + `BudgetSubview`/`SISubview`/`TaxSubview` + stale-ID
      fallback rules in `Sources/FinanceWorkspaceApp/AppRouter.swift` (data-model.md)
- [X] T011 [US1] Implement `RouteActivityCodec` (versioned `NSUserActivity` user-info dict,
...
- [X] T012 [P] [US1] Write `Tests/FinanceWorkspaceAppTests/AppRouterTests.swift` — route⇄activity
      round-trip for every case, stale-entity fallback to parent module, KPI→route mapping
- [X] T013 [US1] Extract and extend `AppState` into `Sources/FinanceWorkspaceApp/AppState.swift`
...
- [X] T014 [US1] Implement `AppRouter.navigate(to:)` + `route(forKPI:)` (sidebar/breadcrumb/pane
      sync; pane closes or re-scopes on navigation) in
      `Sources/FinanceWorkspaceApp/AppRouter.swift`
- [X] T015 [US1] Build `NavigationSidebarView` in
...
- [X] T016 [P] [US1] Build `GlobalHeaderView` in
...
- [X] T017 [P] [US1] Build `BreadcrumbView` + `PageTitleActionsView` (right-aligned per-view
      local actions incl. disabled write actions) in
      `Sources/FinanceWorkspaceApp/UI/Shell/PageTitleActionsView.swift`
- [X] T018 [US1] Build `DetailPaneView` via `.inspector(isPresented:)` (360–420 width, closed by
      ...
      (FR-006, research D1)
- [X] T019 [US1] Rewrite `Sources/FinanceWorkspaceApp/FinanceWorkspaceApp.swift` — three-column
...
- [X] T020 [P] [US1] Write `Tests/FinanceWorkspaceAppTests/CommandMatrixTests.swift` — menu
      enable/disable matrix (Phase-6 commands disabled; selection-context commands gated)
- [X] T021 [US1] Shell checkpoint: run against fixture + empty workspaces (US1 acceptance
...
- [X] T022 [P] [US2] Build `KPICardView` (`KPICardModel`: overline, tabular value, delta
      pos/neg/flat, whole-card tap → route, typed-state rendering) in
      `Sources/FinanceWorkspaceApp/UI/Shared/KPICardView.swift`
- [X] T023 [P] [US2] Build `DataTableView` (column specs, sticky uppercase header, 30 px rows,
...
- [X] T024 [P] [US2] Build `PieChartView` (SectorMark donut + legend/labels), `SparklineView`
...
- [X] T025 [P] [US2] Build `HeatMapTableView` (`Grid`-based: sticky row headers, 8 period
...
- [X] T026 [P] [US2] Build `PeriodSelectorView` (month/quarter/year + prev/next, keyboard
      operable, session-scoped binding) in
      `Sources/FinanceWorkspaceApp/UI/Shared/PeriodSelectorView.swift`
- [X] T027 [P] [US2] Build `EmptyStateView` + `LoadingSkeletonView` in
      `Sources/FinanceWorkspaceApp/UI/Shared/EmptyStateView.swift`
- [X] T028 [P] [US2] Build `SourceInspectorView` (path, row, last-modified, raw fields; "Open in
...
- [X] T029 [P] [US2] Build `ValueProvenanceLabel` (imported/derived/repaired/user-edited) in
      `Sources/FinanceWorkspaceApp/UI/Shared/ValueProvenanceLabel.swift` (FR-013)
- [X] T030 [US2] Build `LedgerTableView` — `LedgerEntry` grouping by `group_id` (summary row +
...
- [X] T031 [US2] Component checkpoint: light+dark `#Preview`s + empty states on every component;
...
- [X] T032 [US3] Implement `OverviewViewModel` (dashboard → 5 `KPICardModel`s incl. `RateState`
...
- [X] T033 [P] [US3] Write `Tests/FinanceWorkspaceAppTests/OverviewViewModelTests.swift` —
      card mapping (incl. "rate not set"), MoM series, issue grouping/count = chip count
- [X] T034 [US3] Build `OverviewView` (KPI grid, no filters, MoM panel, inline issues table;
      KPI tap → module route) in `Sources/FinanceWorkspaceApp/UI/Overview/OverviewView.swift`
- [X] T035 [US3] Build `OverviewIssuesTableView` + "Preview Repair" → `RepairService` dry-run →
...
- [X] T036 [US3] Overview checkpoint: reconcile on-screen values with `overview-dashboard`
...
- [X] T037 [US4] Implement `AccountsViewModel` (aggregate assets/liabilities header, group
...
- [X] T038 [P] [US4] Write `Tests/FinanceWorkspaceAppTests/AccountsViewModelTests.swift` —
      aggregate math passthrough (no view-side computation), multi-entry grouping, business
      P&L section presence per `group_type`
- [X] T039 [US4] Build `AccountsView` (card grid grouped by account group, aggregate net-worth
      header, card → account, group header → group) in
      `Sources/FinanceWorkspaceApp/UI/Accounts/AccountsView.swift` (FR-017)
- [X] T040 [US4] Build `AccountGroupDetailView` (account cards above inline `LedgerTableView`,
...
- [X] T041 [US4] Build `AccountDetailView` (transactions ledger, monthly gross vs expenses/tax
...
- [X] T042 [US4] Accounts checkpoint: reconcile with `accounts-overview` (SC-004); US4
...
- [X] T043 [US5] Implement `BudgetViewModel` (pie slices, spend-mix + variance panel models,
...
- [X] T044 [P] [US5] Write `Tests/FinanceWorkspaceAppTests/BudgetViewModelTests.swift` — period
      switch recompute, partial-aware trailing-average passthrough, drill-down filter mapping
- [X] T045 [US5] Build `BudgetOverviewView` (PieChartView, Spend Mix / Spending Variance at
...
- [X] T046 [US5] Build `BudgetHistoryView` (MoM variance over a period range) +
...
- [X] T047 [US5] Budget checkpoint: reconcile with `budget-overview` for ≥2 periods (SC-004);
...
- [X] T048 [US6] Implement `SavingsInvestmentsViewModel` (goal cards/detail, holdings rows with
...
- [X] T050 [US6] Build `SavingsInvestmentsView` (Overview/Goals/Portfolio sub-navigation, no
      "Categories") in
      `Sources/FinanceWorkspaceApp/UI/SavingsInvestments/SavingsInvestmentsView.swift` (FR-023)
- [X] T051 [US6] Build `GoalsListView` (flat goal cards + progress bars) + `GoalDetailView`
...
- [X] T052 [US6] Build `PortfolioView` (holdings `DataTableView` primary, standard ⇄ heat-map
...
- [X] T053 [US6] Build `HoldingDetailView` (security detail, FIFO tax-lot drill-down, trade
...
- [X] T054 [US6] S&I checkpoint: reconcile with `savings-overview` / `portfolio-overview` /
...
- [X] T055 [US7] Implement `TaxesViewModel` (current-year sections; deductions with both
...
- [X] T056 [P] [US7] Write `Tests/FinanceWorkspaceAppTests/TaxesViewModelTests.swift` — both
...
- [X] T057 [US7] Build `CurrentTaxYearView` (YTD income, paid vs owed with overrides,
...
- [X] T058 [US7] Build `TaxPrepChecklistView` (full-width, complete/incomplete/missing, source
...
- [X] T059 [US7] Taxes checkpoint: reconcile with `tax-overview` (SC-004); US7 acceptance
...
- [X] T060 [P] [US8] Create `App/project.yml` (XcodeGen: `FinanceWorkspace` macOS app target,
...
- [X] T061 [US8] Add the XcodeGen generate + unsigned `xcodebuild` step to
      `.github/workflows/ci-macos.yml` after `swift test` (CODE_SIGNING_ALLOWED=NO); verify CI
      green (SC-009)
- [X] T062 [US8] Traceability walkthrough: for each of the five modules, verify
...
- [X] T063 [US8] Execute the Milestone-5 demo script (quickstart.md) end-to-end: fixture +
...
- [X] T064 [P] Run `design-token-sync` across DESIGN.md ⇄ `prototype/styles.css` ⇄
...
- [X] T065 [P] Update `docs/test-plans.md` — app is now user-testable: record the Phase-5 user
      flows as testable, note the demo-script results, list still-blocked (write) flows
- [X] T066 [P] Update `docs/out-of-scope-followups.md` — items skipped/deferred during 006
      (attributed to spec + task), e.g. edit-form surface unreachable until Phase 6, XCUITest →
      Phase 7
- [X] T067 Update `docs/project-management.md` (close the Phase-5 `[DECIDE]`s with their
      resolutions), `docs/product-roadmap.md` Phase-5 checkboxes, and
      `docs/technical-design.md` §17 (final menu shortcut table per research D5); refresh the
      CLAUDE.md SPECKIT block + persistent memory (`project-state`,
      `active-spec-006-presentation-layer`) to "build complete"
- [X] T068 Full gate pass: `swift build`, `swift test` (CI), `swiftlint --strict` (CI), and the
